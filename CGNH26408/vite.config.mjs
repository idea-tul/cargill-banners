...
export default defineConfig({
  plugins: [
    ...
    copyBannerScripts(),
    viteImagemin({
      // Disable the on-disk cache: with skipIfLarger (keep the smaller of
      // source vs re-encoded), a warm cache restores the LARGER re-encoded
      // copy on rebuilds, making bundle sizes nondeterministic. Re-optimizing
      // each build keeps output minimal and reproducible.
      cache: false,
      plugins: {
  ...
  base: "./",
  build: {
    // Ad-network packages must keep image layers as physical files. Vite's
    // default 4 KB threshold would otherwise inline small CTA PNGs as base64.
    assetsInlineLimit: 0,
    rollupOptions: {
